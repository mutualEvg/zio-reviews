...
import com.nathan.reviewboard.reposirories.CompanyRepositorySpec.dataSourceLayer
import com.nathan.reviewboard.repositories.{Repository, ReviewRepository, ReviewRepositoryLive}
  ...
  override def spec: Spec[TestEnvironment & Scope, Any] =
    suite("ReviewRepositorySpec")(
      ...
      test("get review by ids (id, companyId, userId)") {
        ...
        } yield (review, fetchedReview, fetchedReview2, fetchedReview3)

        program.assert {
          case (review, fetchedReview, fetchedReview2, fetchedReview3) =>
            fetchedReview.contains(review) &&
              fetchedReview2.contains(review) &&
              fetchedReview3.contains(review)
        }
      },
      test("get review by ids (id, companyId, userId) v2") {
        for {
          repo           <- ZIO.service[ReviewRepository]
          review         <- repo.create(goodReview)
          fetchedReview  <- repo.getById(review.id)
